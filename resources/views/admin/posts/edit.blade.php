...
@section('content')
    <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
            <h1 class="text-2xl font-semibold leading-6 text-gray-900">Modifier l'article</h1>
            <p class="mt-2 text-sm text-gray-700">Mettez à jour les informations de "{{ $post->title }}".</p>
        </div>
    ...
    <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 divide-y divide-gray-200">
        @csrf
        @method('PUT')
        <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
            <div class="space-y-6 sm:space-y-5">
                
                <x-translatable-input name="title" label="Titre" :value="$post->getTranslations('title')" :required="true" />

                <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                    <label for="slug" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">Slug</label>
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $post->slug) }}" required class="max-w-lg block w-full shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md">
                ...
                <x-translatable-textarea name="excerpt" label="Extrait" :value="$post->getTranslations('excerpt')" />

                <x-translatable-textarea name="content" label="Contenu" :value="$post->getTranslations('content')" :rows="10" :required="true" wysiwyg />
                ...
                <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                        @if($post->featured_image)
                            <div class="mb-4">
                                <img src="{{ asset('storage/' . $post->featured_image) }}" alt="Image" class="h-32 w-auto object-cover rounded-md">
                            </div>
                        @endif
                ...
                <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <input type="date" name="published_at" id="published_at" value="{{ old('published_at', $post->published_at ? $post->published_at->format('Y-m-d') : '') }}" class="max-w-lg block w-full shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md">
                 ...
                 <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $post->meta_title) }}" class="max-w-lg block w-full shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:max-w-xs sm:text-sm border-gray-300 rounded-md">
                    </div>
                </div>
                 ...
                 <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                    ...
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                         <textarea id="meta_description" name="meta_description" rows="2" class="max-w-lg shadow-sm block w-full focus:ring-primary-500 focus:border-primary-500 sm:text-sm border border-gray-300 rounded-md">{{ old('meta_description', $post->meta_description) }}</textarea>
                    </div>
                </div>
