<header x-data="{ mobileMenuOpen: false }" class="bg-white shadow-sm sticky top-0 z-50 transition-all duration-300">
    <nav class="container-custom mx-auto flex items-center justify-between h-20" aria-label="Global">
        ...
        <div class="hidden lg:flex lg:flex-1 lg:justify-end gap-4 items-center">
            ...
            <a href="{{ route('contact') }}" class="btn-primary text-sm px-4 py-2 uppercase tracking-wide">{{ __('Demander une démo') }}</a>
        </div>
    </nav>
        ...
        <div class="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div class="mt-6 flow-root">
                <div class="-my-6 divide-y divide-gray-500/10">
                    ...
                    <div class="py-6">
                        <a href="{{ route('contact') }}" class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-bold leading-7 text-primary-600 hover:bg-gray-50 uppercase">{{ __('Demander une démo') }}</a>
                    </div>
                </div>
            </div>
        </div>
