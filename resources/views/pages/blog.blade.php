...
@section('content')
    <section class="bg-primary-900 py-20 text-white">
        <div class="container-custom text-center">
            <h1 class="text-4xl lg:text-5xl font-display font-bold mb-6">{{ __('Le Blog Zygnixis') }}</h1>
            <p class="text-xl text-gray-200 max-w-2xl mx-auto mb-8">
                {{ __('Actualités, conseils et expertises sur Odoo, la transformation digitale et la sécurité') }}
            </p>
            ...
            <form action="{{ route('blog') }}" method="GET" class="max-w-2xl mx-auto">
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" 
                           ...
                           class="w-full px-6 py-4 pr-32 rounded-full text-gray-900 focus:outline-none focus:ring-2 focus:ring-accent-500">
                    <button type="submit" class="absolute right-2 top-2 bg-accent-500 text-white px-6 py-2 rounded-full hover:bg-accent-600 transition-colors">
    ...
    <section class="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
        <div class="container-custom py-4">
            <div class="flex flex-wrap gap-3 justify-center">
                <a href="{{ route('blog') }}" 
                   class="px-4 py-2 rounded-full {{ !request('category') ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} transition-colors">
                    {{ __('Tous') }}
                </a>
                @foreach($categories as $cat)
                    <a href="{{ route('blog', ['category' => $cat->slug]) }}" 
                       class="px-4 py-2 rounded-full {{ request('category') == $cat->slug ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} transition-colors">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
            ...
            <div class="lg:grid lg:grid-cols-4 gap-12">
                    ...
                    <div class="grid md:grid-cols-2 gap-8">
                                ...
                                <div class="flex-1 p-6 flex flex-col">
                                    ...
                                    <div class="mt-auto">
                                        <a href="{{ route('blog.show', $post->slug) }}" class="text-secondary-600 font-semibold text-sm hover:text-secondary-700 inline-flex items-center">
                                            {{ __('Lire l\'article') }} <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                                        </a>
                                    </div>
                                </div>
                            </article>
                        @empty
                            <div class="col-span-full py-12 text-center text-gray-500">
                                {{ __('Aucun article publié pour le moment.') }}
                            </div>
                        @endforelse
                    </div>
                ...
                <aside class="lg:col-span-1 space-y-8 mt-12 lg:mt-0">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        <h3 class="font-bold text-gray-900 mb-4 text-lg">{{ __('Recherche') }}</h3>
                        <form action="#" class="relative">
                            <input type="text" placeholder="{{ __('Mots-clés...') }}" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500 pl-4 pr-10">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        <h3 class="font-bold text-gray-900 mb-4 text-lg">{{ __('Catégories') }}</h3>
                        <ul class="space-y-2">
                    ...
                    <div class="bg-primary-50 p-6 rounded-xl border border-primary-100">
                        <h3 class="font-bold text-primary-900 mb-2">{{ __('Newsletter') }}</h3>
                        <p class="text-sm text-gray-600 mb-4">{{ __('Recevez nos derniers conseils directement dans votre boîte mail.') }}</p>
                        <form action="#">
                            <input type="email" placeholder="{{ __('Entrez votre email') }}" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500 mb-3">
                            <button type="submit" class="w-full btn-primary text-sm">{{ __('S\'abonner') }}</button>
                        </form>
                    </div>
                </aside>
