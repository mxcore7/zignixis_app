...
@section('title', __('Accueil - Transformation Digitale & Odoo'))
...
@section('content')
        ...
        <div class="container-custom relative z-10 py-24 lg:py-32">
            <div class="max-w-2xl animate-fade-in-up">
                <h1 class="text-4xl lg:text-6xl font-display font-bold mb-6 leading-tight">
                    {!! __("Accélérez votre <span class=\"text-secondary-500\">Transformation Digitale</span> avec l'ERP Odoo") !!}
                </h1>
    ...
    <section class="py-20 bg-gray-50">
        <div class="container-custom">
            <div class="text-center max-w-3xl mx-auto mb-16">
                <h2 class="text-3xl font-display font-bold text-primary-900 mb-4">{{ __("Nos Domaines d'Expertise") }}</h2>
                <p class="text-gray-600">{{ __("Nous combinons expertise technique et compréhension métier pour propulser votre entreprise vers l'excellence opérationnelle.") }}</p>
            </div>
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 border border-gray-100 group">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 mb-3">{{ __('Intégration ERP Odoo') }}</h3>
                    <p class="text-gray-600 mb-4">{{ __("Comptabilité, RH, Ventes, Stocks. Une suite d'applications intégrées pour gérer toute votre entreprise.") }}</p>
                    <a href="{{ route('solutions') }}" class="text-primary-600 font-semibold hover:text-primary-700 flex items-center">{{ __('En savoir plus') }} <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg></a>
                </div>
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 border border-gray-100 group">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 mb-3">{{ __('Sécurité Électronique') }}</h3>
                    <p class="text-gray-600 mb-4">{{ __("Vidéosurveillance (CCTV), Contrôle d'accès, Systèmes d'alarme. Protégez vos actifs avec les dernières technologies.") }}</p>
                    <a href="{{ route('solutions') }}" class="text-secondary-600 font-semibold hover:text-secondary-700 flex items-center">{{ __('En savoir plus') }} <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg></a>
                </div>
                ...
                <div class="bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl transition-shadow duration-300 border border-gray-100 group">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 mb-3">{{ __('Développement Sur Mesure') }}</h3>
                    <p class="text-gray-600 mb-4">{{ __("Applications Web & Mobile, Sites Corporate, E-commerce. Des solutions digitales performantes et évolutives.") }}</p>
                    <a href="{{ route('solutions') }}" class="text-accent-600 font-semibold hover:text-accent-700 flex items-center">{{ __('En savoir plus') }} <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg></a>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-20 bg-white">
        <div class="container-custom">
            <div class="lg:grid lg:grid-cols-2 lg:gap-16 items-center">
                ...
                <div>
                    <span class="text-secondary-600 font-bold tracking-wider uppercase text-sm mb-2 block">{{ __('Pourquoi choisir Odoo ?') }}</span>
                    <h2 class="text-3xl lg:text-4xl font-display font-bold text-primary-900 mb-6">{{ __('La solution tout-en-un pour gérer votre entreprise') }}</h2>
                    <p class="text-gray-600 mb-8 text-lg">
                        {{ __("Fini les logiciels dispersés et les données en silos. Odoo centralise toutes vos opérations dans une interface unique, moderne et facile à utiliser.") }}
                    </p>
                    ...
                    <ul class="space-y-4 mb-10">
                        <li class="flex items-start">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-900">{{ __('Modulaire et Évolutif') }}</h4>
                                <p class="text-gray-600 text-sm">{{ __("Commencez petit avec quelques applications et ajoutez-en d'autres au fur et à mesure de votre croissance.") }}</p>
                            </div>
                        </li>
                        <li class="flex items-start">
                            <svg class="w-6 h-6 text-green-500 mr-3 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                            <div>
                                <h4 class="font-bold text-gray-900">{{ __('Interface Intuitive') }}</h4>
                                <p class="text-gray-600 text-sm">{{ __("Une expérience utilisateur moderne qui réduit drastiquement le temps de formation de vos employés.") }}</p>
                            </div>
                        </li>
                        <li class="flex items-start">
                            <svg class="w-6 h-6 text-green-500 mr-3 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                            <div>
                                <h4 class="font-bold text-gray-900">{{ __('Coût Optimisé') }}</h4>
                                <p class="text-gray-600 text-sm">{{ __("Un excellent rapport qualité/prix comparé aux ERP traditionnels comme SAP ou Oracle.") }}</p>
                            </div>
                        </li>
                    </ul>
    ...
    <section class="py-20 bg-gray-50">
        <div class="container-custom">
            <div class="flex justify-between items-end mb-12">
                <div>
                    <h2 class="text-3xl font-display font-bold text-primary-900 mb-2">{{ __('Nos Récentes Réalisations') }}</h2>
                    <p class="text-gray-600">{{ __('Découvrez comment nous aidons nos clients à réussir.') }}</p>
                </div>
                <a href="{{ route('projects') }}" class="hidden md:flex text-primary-600 font-semibold hover:text-primary-800 items-center">
                    {{ __('Voir tout') }} <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                </a>
            </div>
                    ...
                    <div class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300">
                        <div class="h-48 bg-gray-200 relative">
                            ...
                            <div class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-primary-900 shadow-sm">
                                {{ $project->sector ?? __('Technologie') }}
                            </div>
                        </div>
                        <div class="p-6">
                            <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $project->title }}</h3>
                            <p class="text-gray-600 text-sm mb-4 line-clamp-2">{{ Str::limit($project->description, 100) }}</p>
                            <a href="{{ route('projects.show', $project->slug) }}" class="text-secondary-600 font-semibold text-sm hover:text-secondary-700">{!! __('Voir les détails &rarr;') !!}</a>
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="mt-8 text-center md:hidden">
                <a href="{{ route('projects') }}" class="btn-outline">{{ __('Voir tous nos projets') }}</a>
            </div>
        </div>
    </section>
    @endif
    ...
    <section class="py-20 bg-primary-900 text-white relative overflow-hidden">
        ...
        <div class="container-custom relative z-10 text-center max-w-4xl mx-auto">
            <h2 class="text-3xl lg:text-5xl font-display font-bold mb-6">{{ __('Prêt à transformer votre entreprise ?') }}</h2>
            <p class="text-xl text-gray-200 mb-10">
                {{ __("Nos experts sont prêts à auditer vos besoins et vous proposer la solution idéale. La première consultation est gratuite.") }}
            </p>
